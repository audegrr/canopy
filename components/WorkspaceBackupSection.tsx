...
import { backupFilename, collectAssetUrls, MAX_BACKUP_BYTES, parseWorkspaceBackup, type WorkspaceBackup } from '@/lib/workspace-backup'
...
export default function WorkspaceBackupSection({ workspace }: { workspace: Workspace }) {
  ...
  useEffect(() => {
    setLastExportAt(localStorage.getItem(`canopy_backup_${workspace.id}`))
  }, [workspace.id])
  const lastExportLabel = lastExportAt
    ? `Last export on this device: ${new Date(lastExportAt).toLocaleString('en')}`
    : 'No export has been recorded on this device.'
  ...
  async function exportWorkspace() {
    ...
    try {
      ...
      if (error || !pages) throw error ?? new Error('Workspace pages could not be read.')
      const pageIds = pages.map(page => page.id)
      const supplemental = pageIds.length ? await Promise.all([
      ...
      const failedQuery = supplemental.find(result => result.error)
      if (failedQuery?.error) throw failedQuery.error
      const [{ data: fields }, { data: records }, { data: comments }, { data: snapshots }] = supplemental
      const exportedAt = new Date().toISOString()
      const backup: WorkspaceBackup = {
        format: 'canopy-workspace', version: 1, exported_at: exportedAt,
      ...
      localStorage.setItem(`canopy_backup_${workspace.id}`, exportedAt)
      setLastExportAt(exportedAt)
      setMessage(`${pages.length} page${pages.length === 1 ? '' : 's'} exported.`)
    } catch (error) {
      reportClientError(error, { operation: 'export_workspace_backup' })
      setMessage('Export failed. No backup file was created.')
    } finally {
      setBusy(null)
    }
  }
  ...
  async function importWorkspace(file: File) {
    ...
    try {
      if (file.size > MAX_BACKUP_BYTES) throw new Error('Backup exceeds the 15 MB import limit.')
      ...
      reportClientError(error, { operation: 'import_workspace_backup' })
      let cleanupFailed = false
      if (createdPageIds.length) {
        const supabase = createClient()
        for (const pageId of [...createdPageIds].reverse()) {
          const { error: cleanupError } = await supabase.from('pages').delete().eq('id', pageId)
      ...
      setMessage(cleanupFailed
        ? `${detail} Some imported items could not be removed; review this workspace before retrying.`
        : `${detail}${createdPageIds.length ? ' Everything created by this attempt was removed.' : ''}`)
      setBusy(null)
  ...
  return <div style={{ marginTop: 22, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
    ...
    <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 10 }}>{lastExportLabel}</div>
