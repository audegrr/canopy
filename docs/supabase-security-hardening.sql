...
begin
  if to_regprocedure('public.get_auth_user_by_email(text)') is not null then
    ...
    raise notice 'Skipping get_auth_user_by_email(text): function does not exist';
  end if;
  ...
  if to_regprocedure('public.get_unconfirmed_auth_user_id(text)') is not null then
    ...
    raise notice 'Skipping get_unconfirmed_auth_user_id(text): function does not exist';
  end if;
  ...
  if to_regprocedure('public.get_workspace_pages(uuid)') is not null then
    ...
    raise notice 'Skipping get_workspace_pages(uuid): function does not exist';
  end if;
end $$;
